...
import { BlobStore,StorageListObjectInterface, StorageObjectInterface, StorageProviderInterface,} from '../../../common/interfaces/storageProvider';
...
export class LocalStorage implements StorageProviderInterface {
    ...
    constructor() {
        ...
        if (
            !fs.existsSync(
                path.join(appRootPath.path, 'upload')
            )
        )
    ...
    getObject = async (key: string): Promise<StorageObjectInterface> => {
        const filePath = path.join(
            appRootPath.path,this.path,key
        );
    ...
    putObject = async (params: StorageObjectInterface): Promise<any> => {
        ...
        if (pathWithoutFile == null)
            throw new Error('Invalid file path in local putObject');
    ...
    getSignedUrl = (key: string, expiresAfter: number, conditions:any): any => {
        return {
            fields: {
                Key: key,
            },
